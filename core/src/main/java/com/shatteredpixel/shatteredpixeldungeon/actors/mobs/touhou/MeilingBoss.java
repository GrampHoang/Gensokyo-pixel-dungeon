...
public class MeilingBoss extends Mob {
    ...
    private float rock_cd = 5;
	private float punch_cd = 2;
	...
	public void damage(int dmg, Object src) {
		if ((HP*3 <= HT)){
			...
			yell(Messages.get(this, "Meiling is getting desperated!"));
		}
        ...
        if (punch_pos_drop != null){
            for (int i : punch_pos_drop){
                if (i != 0){
                    ...
                    if (ch != null && !(ch instanceof MeilingBoss)){
		...
		public boolean act( boolean enemyInFOV, boolean justAlerted ) {
			...
			if (leapPos != -1){
				...
				sprite.jump(pos, leapPos, new Callback() {
					...
					public void call() {
						...
						leapPos = -1;
						sprite.idle();
						Dungeon.level.occupyCell(MeilingBoss.this);
						next();
					}
				});
				return false;
            ...
            if (canUseAbility()){
				...
				if (enemyInFOV) {
					...
					state = WANDERING;
					target = Dungeon.level.randomDestination( MeilingBoss.this );
					return true;
				}
