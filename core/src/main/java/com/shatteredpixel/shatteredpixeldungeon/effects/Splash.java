...
public class Splash {
	...
	public static void atExplode( int cell, final int color, int n ) {
		atExplode( DungeonTilemap.tileCenterToWorld( cell ), color, n );
	}
	
	public static void atExplode( PointF p, final int color, int n ) {
		...
		EXPLODE.color = color;
		EXPLODE.dir = -3.1415926f / 2;
		EXPLODE.cone = 3.1415926f;
		emitter.burst( EXPLODE, n );
	}
	
	public static void atExplode( PointF p, final float dir, final float cone, final int color, int n ) {
	...
	public static void atExplode( PointF p, final float dir, final float cone, final int color, int n, float interval ) {
		...
		EXPLODE.dir = dir;
		EXPLODE.cone = cone;
		emitter.start( EXPLODE, interval, n );
	}

	private static final SplashExplode EXPLODE = new SplashExplode();
			
	private static class SplashExplode extends Emitter.Factory {
		...
		public void emit( Emitter emitter, int index, float x, float y ) {
			...
			p.reset( x, y, color, 12, Random.Float( 0.1f, 0.3f ) );
			p.speed.polar( Random.Float( dir - cone / 2, dir + cone / 2 ), Random.Float( 40, 80 ) );
			p.acc.set( 0, +100 );
		}
	}
